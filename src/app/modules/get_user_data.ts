...
export default async function (config: any) {
    ...
    const installationsRequest = await fetch(`https://api.github.com/user/installations`, {
        headers: {
            "Authorization": `Bearer ${token}`,
            "X-GitHub-Api-Version": "2022-11-28",
            "Accept": "application/vnd.github+json"
    ...
    const repoRequest = await fetch(`https://api.github.com/user/installations/${installationId}/repositories`, {
        headers: {
            "Authorization": `Bearer ${token}`,
            "X-GitHub-Api-Version": "2022-11-28",
            "Accept": "application/vnd.github+json"
